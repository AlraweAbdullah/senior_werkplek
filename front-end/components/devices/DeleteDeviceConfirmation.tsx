...
const DeleteDevice: React.FC<Props> = ({ device }: Props) => {
    const router = useRouter();
    const userId = parseInt(router.query.userId as string);
    const [statusMessage, setStatusMessage] = useState(null);

    const handleSubmitThrowAway = async (
        event: React.MouseEvent<HTMLButtonElement>
    ) => {
        event.preventDefault();
        const throwAwayInput: DeviceInput = { status: 'throwAway' };

        const response = await DeviceService.changeDeviceStatus(device.id, throwAwayInput);
    ...
    const handleSubmitDonate = async (
        ...
        event.preventDefault();
        const donateInput: DeviceInput = { status: 'donate' };
        const response = await DeviceService.changeDeviceStatus(device.id, donateInput);
        const data = await response.json();

        if (response.status === 200) {
            setStatusMessage({
                type: 'success',
                message: `Uw toestel is klaar voor donatie.`,
            });
            ...
            {device && (
                <>
                    ...
                    <Button
                        ...
                        className="px-4 fs-6 ms-2"
                    >
                        <FontAwesomeIcon size="xs" icon={faGift} /> Toestel doneren
                    </Button>
                </>
            )}
        </div>
